...
var MidiPlayer = (function () {
  ...
  var Track = /*#__PURE__*/ (function () {
    function Track(index, data) {
      ...
      var lastThreeBytes = this.data.subarray(
        this.data.length - 3,
        this.data.length
      );
      ...
      if (
        !(
          lastThreeBytes[0] === 0xff &&
          lastThreeBytes[1] === 0x2f &&
          lastThreeBytes[2] === 0x00
        )
      ) {
        throw (
          "Invalid MIDI file; Last three bytes of track " +
          this.index +
          "must be FF 2F 00 to mark end of track"
        );
      }
    }
    /**
    ...
    _createClass(Track, [
      ...
      },
      {
        ...
        value: function getStringData(eventStartIndex) {
          var varIntLength = Utils.getVarIntLength(
            this.data.subarray(eventStartIndex + 2)
          );
          var varIntValue = Utils.readVarInt(
            this.data.subarray(
              eventStartIndex + 2,
              eventStartIndex + 2 + varIntLength
            )
          );
          var letters = Utils.bytesToLetters(
            this.data.subarray(
              eventStartIndex + 2 + varIntLength,
              eventStartIndex + 2 + varIntLength + varIntValue
            )
          );
          return letters;
        },
      ...
      },
      {
        ...
        value: function parseEvent() {
          ...
          if (this.data[eventStartIndex] == 0xff) {
            ...
            switch (this.data[eventStartIndex + 1]) {
              ...
              case 0x58:
                ...
                eventJson.timeSignature =
                  "" + eventJson.data[0] + "/" + Math.pow(2, eventJson.data[1]);
                break;
            ...
            var varQuantityByteLength = Utils.getVarIntLength(
              this.data.subarray(eventStartIndex + 1)
            );
            var varQuantityByteValue = Utils.readVarInt(
              this.data.subarray(
                eventStartIndex + 1,
            ...
            this.pointer +=
              deltaByteCount + 1 + varQuantityByteLength + varQuantityByteValue;
          } else if (this.data[eventStartIndex] === 0xf7) {
            // Sysex (escape)
            // http://www.somascape.org/midi/tech/mfile.html#sysex
            eventJson.name = "Sysex (escape)";

            var _varQuantityByteLength = Utils.getVarIntLength(
              this.data.subarray(eventStartIndex + 1)
            );

            var _varQuantityByteValue = Utils.readVarInt(
              this.data.subarray(
                eventStartIndex + 1,
                eventStartIndex + 1 + _varQuantityByteLength
              )
            );
            ...
            eventJson.data = this.data.subarray(
              ...
              eventStartIndex +
                1 +
                _varQuantityByteLength +
                _varQuantityByteValue
            );
            this.pointer +=
              deltaByteCount +
              1 +
    ...
    _createClass(Player, [
      ...
      },
      {
        ...
        value: function getTracks() {
          ...
          this.midiChunksByteLength =
            Constants.HEADER_CHUNK_LENGTH + trackChunksByteLength;
          return this;
        },
        /**
      ...
      },
      {
        ...
        value: function getDivision() {
          this.division = Utils.bytesToNumber(
            this.buffer.subarray(12, Constants.HEADER_CHUNK_LENGTH)
          );
          return this;
        },
        /**
